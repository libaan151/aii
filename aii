...
if not shared.SilentAim then --// If the table doesn't exist. Create one.
    shared.SilentAim = {
        ...
        ACTION_NAME = "Toggle",
        ACTION_KEYCODE = Enum.KeyCode.End,
        
        ignoreList = {
            workspace.Characters, 
            workspace.NonGeometry, 
            workspace.Terrain,
        },
    }
end
...
local function handleAction(actionName, inputState, inputObject)
	if actionName == shared.SilentAim.ACTION_NAME and inputState == Enum.UserInputState.Begin then
		shared.SilentAim.Enabled = not shared.SilentAim.Enabled
	end
end
...
local function GetClosestPlayerCharacterToCursor()
    ...
    for Index, Player in pairs(Players:GetPlayers()) do --// Loop through all the players.
        ...
        if HeadPart then
            ...
            local checkParts = workspace.CurrentCamera:GetPartsObscuringTarget({HeadPart.Position}, shared.SilentAim.ignoreList)
...
TS.Projectiles.InitProjectile = function(self, ...) --// I can't explain self well, but in this situation it is just the Projectiles table. Hooking the function.
    ...
    if shared.SilentAim.Enabled and Arguments[4] == LocalPlayer then
        ...
        if ClosestPlayer and (ClosestPlayer.Health.Value > 0) then --// Check if it returned a player and the player is alive.
            if shar then --// If the random number hits 100 we goto the head because it got the top number.
...
ContextActionService:BindAction(shared.SilentAim.ACTION_NAME, handleAction, false, shared.SilentAim.ACTION_KEYCODE)
